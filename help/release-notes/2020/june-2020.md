...
| Real-time Machine Learning (Alpha). | Provides the ability to author, test and deploy custom or imported pre-trained machine learning models in industry-standard interoperable model formats for real-time scoring/activation via an API endpoint. |

For more information on Real-time Machine Learning alpha, please see the [Real-time Machine Learning overview](../../data-science-workspace/recipe-notebook-migration.md).
