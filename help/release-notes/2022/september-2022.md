...
| API support for retrying failed dataflow runs | Use the `re-trigger` operation to retry your failed dataflow through the API. Support for retrying failed dataflow runs is available to batch sources. Read the guide on [retrying failed dataflow runs using the API](../../sources/tutorials/api/retry-flows.md) for more information. |
