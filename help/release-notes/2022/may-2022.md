...
| Feature | Description |
| --- | --- |
| Resources added for Access control policy, Role, Audit logs, Work order, Identity name space, Identity graph, Query, Dataset, and Source data flow added. | Additional resources which are recorded by audit logs. If the feature is enabled, the audit logs will be automatically collected as activity occurs. You do not need to manually enable log collection. |
