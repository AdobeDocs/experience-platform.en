...
| Feature | Description |
| --- | --- |
| [[!DNL Adobe Target] connection](/help/destinations/catalog/personalization/adobe-target-connection.md) now available to [!DNL Adobe Journey Optimizer] organizations | The **[!UICONTROL Adobe Target]** destination is now available to organizations with [!DNL Adobe Journey Optimizer], in addition to organizations with [!DNL Real-Time CDP]. No customer action is required for eligible organizations to see the destination card in the catalog. See the [Entitlement](/help/destinations/catalog/personalization/adobe-target-connection.md#entitlement) section for details. |
