...
| Adobe Audience Manager to Experience Platform data sharing | The provisioning process for the Audience Manager connector has been updated. Audience Manager datasets for Profile are now disabled by default. You can manually choose which datasets to promote to the Real-time CDP The new default settings are not retroactive and only impact provisioning for new Audience Manager connectors. See more information in the [Audience Manager](../../sources/tutorials/ui/create/adobe-applications/audience-manager.md) tutorial. |
