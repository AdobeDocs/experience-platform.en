...
| [Use calculated fields to export arrays in flat schema files](/help/destinations/ui/export-arrays-calculated-fields.md) | Support for using calculated fields to export arrays in flat schema files is now generally available. All the functions are generally available, except `array_to_string`, `filterArray`, `transformArray`, and `flattenArray`.  |
