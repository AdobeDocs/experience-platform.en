...
| Feature | Description |
| --- | --- |
| [Control which audiences to export to enterprise destinations](../destinations/ui/activate-segment-streaming-destinations.md#select-destination) | Use the new **[!UICONTROL Include mapped audiences only]** toggle to choose what the `segmentMembership` object contains when you export audiences to the [!DNL HTTP API], [!DNL Amazon Kinesis], and [!DNL Azure Event Hubs] destinations. Turn it on to include only the audiences mapped in that dataflow, or leave it off to include every audience a profile qualifies for. |
| [Find and filter destinations faster](../destinations/ui/activate-segment-streaming-destinations.md#select-destination) | Search by flow name to locate a destination or existing dataflow in the **[!UICONTROL Select destination]** step of the activation workflow. This is especially useful when you activate an audience directly to a destination. <br> ![Searching for a destination or dataflow by flow name in the Select destination step.](assets/search-destinations.gif){zoomable="yes"} |
| [Find and filter audiences faster](../destinations/ui/activate-segment-streaming-destinations.md#select-audiences) | Filter audiences by evaluation type (edge, streaming, or batch), namespace origin, and audience tags in the **[!UICONTROL Select audiences]** step of the activation workflow. <br> ![Filtering audiences by evaluation type, namespace origin, and audience tags in the Select audiences step.](assets/search-audiences.gif){zoomable="yes"} |
