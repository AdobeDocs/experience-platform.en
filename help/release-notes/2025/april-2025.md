...
| Feature | Description |
| --- | --- |
| [!DNL Adform] extension | The [!DNL Adform] server-side extension enables advertisers, media agencies, and publishers to synchronize data directly with Adform. This integration allows businesses to engage audiences across multiple channels by leveraging first-party IDs like ECID. This extension eliminates the need for third-party cookies and client-side JavaScript deployment, enabling seamless audience retargeting. For more information, see the [Adform extension overview](/help/tags/extensions/server/adform/overview.md). |
