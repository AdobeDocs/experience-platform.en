...
The Optical Character Recognition (OCR) service, when given an image, can indicate if text is present in the image. If text is present, OCR can return the text.
...
| `content` | The content used by the OCR service. The content can be raw image (‘inline’ content-type). <br> If the content is a file on S3 ('s3-bucket' content-type), pass the signed url. | Yes |

**Response**

A successful response returns the text that was detected in the `feature_value` array. The text is read and returned top down from left to right. This means that if "I love Adobe" was detected, your payload returns "I", "LOVE", and "Adobe" all in their own object under `feature_name` in that order.
