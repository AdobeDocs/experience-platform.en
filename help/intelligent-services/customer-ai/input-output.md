...
Add link to XDM Schema and XDM Class 

Take out churn definition. 

Customer AI input data (@Jose Mathew’s team) 
...
Standard field groups used by Customer AI (@Jose Mathew’s team) 
...
Adding user-defined events and profile attributes (@Yi-Hong Kuo’s team) 

If you have information you wish to include in addition to the default standard field groups used by Customer AI, you can use the user-defined event configuration to augment the data used by the model.  
...
Incorporating domain knowledge or prior expertise into the model through the addition of data 

Potentially improving the predictive model quality 

Gaining additional insights and interpretability 

Choosing user-defined events to include: 
...
Industry specific user-defined event examples 
...
Add known traveler info Purchase miles 

Communications 

Upgrade/downgrade/cancel plan 

Requirements for choosing user-defined events: 
...
Historical data (@Yi-Hong Kuo’s team) 

Customer AI requires historical data for model training.  The length of data (in terms of timespan) required is based on two key elements: outcome window and eligible population. 
...
Customer AI output data (Bonnie) 

Customer AI generates several attributes for individual profiles that are deemed eligible. There are two ways to consume the score output based on what you have provisioned. If you have a Real-time Customer Profile-enabled dataset, you can consume insights from Real-time Customer Profile in the Segment Builder. If you don’t have a Profile-enabled dataset, you can download the Customer AI output dataset available on the data lake. 

You can find the output dataset under Datasets in Platform. All Customer AI output datasets start with the name Customer AI Scores - Name_of_app. Similarly, all Customer AI output schemas start with the name Customer AI Schema - Name_of_app. The identity fields are found under the IdentityMap.  

@Serah Metpalli Re-use the same screenshot.  

The table below describes the various attributes found in the output of Customer AI: 

@Serah Metpalli Please insert the same table here.  

 

Governance, privacy, and security for Customer AI (Bonnie) 
