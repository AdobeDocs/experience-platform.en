...
With the updates to Spark recipes, a number of values need to be added and changed. First, `DataSetOptions` is no longer used. Replace `DataSetOptions` with `QSOption`. Additionally, new `option` parameters are required. `QSOption.datasetId` is needed and replaces the need to load the `{dataSetId}` in `.save()`. Lastly, `orgId` and `serviceApiKey` need to be changed to `imsOrg` and `apiKey`. See the table below for a comparison on writing datasets:
...
Your folder structure should now look similar to the following example. The new files that have been added are highlighted in the image below:
