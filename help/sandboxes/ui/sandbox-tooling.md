...
| Platform | Object | Dependent Objects Supported | Details |
| --- | --- | --- | --- |
...
| [!DNL Adobe Journey Optimizer] | Journey | Below objects used in the journey are copied as dependent objects. You can select **[!UICONTROL Create new]** or **[!UICONTROL Use existing]** during the importing workflow. <ul><li>Audiences</li><li>Schemas</li><li>Custom Actions</li><li>Events</li><li>Fragments</li><li>Content templates</li></ul>| <ul><li>**[!UICONTROL Custom actions]**: When selecting **[!UICONTROL Use existing]** during the import process when copying a journey to another sandbox, the existing custom actions you select **must** be the same as the source custom action. If they are not the same, the new journey will have unresolvable errors.</li><li>The events and event details used in the journey are copied. It will always create a new version in the target sandbox.</li></ul> |
