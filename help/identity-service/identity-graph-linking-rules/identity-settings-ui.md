...
A warning message appears, indicating that existing graphs will only be affected by the graph algorithm only if they get updated **after saving your settings**. Additionally, you are notified that it  will take up to **six hours** for your new settings to take effect. To confirm enter your sandbox name and then select **[!UICONTROL Confirm]**.
