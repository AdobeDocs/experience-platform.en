...
| Phone (E.164) | A namespace that represents raw phone numbers that need to be hashed in E.164 format. To convert raw phone numbers to E.164, prefix a phone number with a plus sign (`+`) and the appropriate country code. |
| Phone (SHA256_E.164) | A namespace that represents phone numbers that need to be hashed using SHA256 without any dashes and under the E.164 format. See the following document on [SHA256 hashing support](https://experienceleague.adobe.com/docs/id-service/using/reference/hashing-support.html?lang=en#hashing-support) for more information. |
