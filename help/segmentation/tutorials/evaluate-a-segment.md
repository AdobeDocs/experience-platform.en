...
If you have not yet completed the [create a segment definition using the Segmentation API](./create-a-segment.md) tutorial or created a segment definition using [Audience Builder](../ui/audience-builder.md), please do so before proceeding with this tutorial.
...
When segment jobs are successfully run, the `segmentMembership` map is updated for each profile included within the segment definition. `segmentMembership` also stores any pre-evaluated audiences that are ingested into [!DNL Experience Platform], allowing for integration with other solutions like [!DNL Adobe Audience Manager].
