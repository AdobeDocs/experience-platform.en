...
| `L` | This value has a different meaning, depending on which field it is used by. If it is used by the day of the month field, it represents the last day of the month. If it is used by the day of the week field by itself, it represents the last day of the week, which is Saturday (`SAT`). If it is used by the day of the week field, in conjunction with another value, it represents the last day of that type for the month. For example, if you put `5L` in the day of the week field, it would **only** include the last Friday of the month. |
...
| `0 30 9 * * ? 2022` | The event will fire ever day at 9:30AM in the year 2022. |
| `0 * 18 * * ?` | The event will fire ever minute, starting at 6PM and ending at 6:59PM, every day. |
