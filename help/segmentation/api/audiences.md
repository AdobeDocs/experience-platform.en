...
## Update multiple audiences {#bulk-patch}

You can update the profile and record count of multiple audiences by making a POST request to the `/audiences/bulk-patch-metric` endpoint and providing the IDs of the audiences you wish to update.
...
+++ A sample request for updating multiple audiences.
...
<td>The ID of the job that will run the update.</td>
</tr>
<tr>
<td><code>jobType</code></td>
<td>The type of job that will run the update. This value can either be <code>export</code> or <code>AO</code>.</td>
</tr>
<tr>
<td><code>audienceId</code></td>
<td>The ID of the audiences you want to update. Please note that this is the <code>audienceId</code> value, and <strong>not</strong> the <code>id</code> value of the audiences.</td>
</tr>
<tr>
<td><code>namespace</code></td>
<td>The namespace for the audience that you want ot update.</td>
</tr>
<tr>
<td><code>operations</code></td>
<td>An object containing the information used to update the audience.</td>
</tr>
<tr>
<td><code>operations.op</code></td>
<td>The operation used for the patch. When updating multiple audiences, this value is <strong>always</strong> <code>add</code>.</td>
</tr>
<tr>
<td><code>operations.path</code></td>
<td>The path of the field to be updated. Currently, only two paths are supported: <code>/metrics/data</code> when you are updating the <strong>profile</strong> count and <code>/recordMetrics/data</code> when you are updating the <strong>record</strong> count.</td>
</tr>
<tr>
<td><code>operations.value</code></td>
<td>
The value of the field to be updated. When you're updating the profile count, this value will look like: 
<pre>
{
    "totalProfiles": 123456
}
</pre>
When you're updating the record count, this value will look like: 
<pre>
{
    "recordCount": 123456
}
...
A successful response returns HTTP status 207 with details about the updated audiences.

+++ A sample response for updating multiple audiences.
...
| Parameter | Description |
| --------- | ----------- |
| `status` | The status of the updated audience. If the returned status is 200, the audience was successfully updated. If the audience could not be updated, an error explaining why the audience was not updated will be returned. |
