...
description: Export jobs are asynchronous processes that are used to persist audience members to datasets. You can use the /export/jobs endpoint in the Adobe Experience Platform Segmentation Service API, which allows you to programmatically retrieve, create, and cancel export jobs.
role: Developer
...
| -------- | ----------- |
| `destination` | Destination information for the exported data:<ul><li>`datasetId`: The ID of the dataset where data was exported.</li><li>`segmentPerBatch`: A Boolean value that shows whether or not segment definition IDs are consolidated. A value of "false" means all the segment definition IDs are exported into a single batch ID. A value of "true" means that one segment definition ID is exported into one batch ID. **Note:** Setting the value to true may affect batch export performance.</li></ul> |
...
| `filter.segments` | The segment definitions that are exported. The following fields are included:<ul><li>`segmentId`: The segment definition ID that profiles will be exported to.</li><li>`segmentNs`: Segment definition namespace for the given `segmentID`.</li><li>`status`: An array of strings providing a status filter for the `segmentID`. By default, `status` will have the value `["realized"]` which represents all profiles that fall into the segment definition at the current time. Possible values include: `realized` and `exited`. A value of `realized` means the profile qualifies for the segment definition. A value of `exiting` means the profile is exiting the segment definition.</li></ul> |
...
| `mergePolicy` | Specifies the merge policy to govern the exported data. Include this parameter when there are multiple segment definitions being exported. If not provided, the export will take the same merge policy as the given segment definition. |
| `filter` | An object that specifies the segment definitions that are going to be included in the export job by ID, qualification time, or ingest time, depending on the subproperties listed below. If left blank, all the data will be exported. |
| `filter.segments` | Specifies the segment definitions to export. Omitting this value will result in all data from all profiles being exported. Accepts an array of segment objects, each containing the following fields:<ul><li>`segmentId`: **(Required if using `segments`)** Segment ID for profiles to be exported.</li><li>`segmentNs` *(Optional)* Segment namespace for the given `segmentID`.</li><li>`status` *(Optional)* An array of strings providing a status filter for the `segmentID`. By default, `status` will have the value `["realized"]` which represents all profiles that fall into the segment definition at the current time. Possible values include: `realized` and `exited`.  A value of `realized` means the profile qualifies for the segment definition. A value of `exiting` means the profile is exiting the segment definition.</li></ul> |
| `filter.segmentQualificationTime` | Filter based on qualification time for the segment definition. The start time and/or end time can be provided. |
| `filter.segmentQualificationTime.startTime` | Qualification start time for a segment definition ID for a given status. It not provided, there will be no filter on the start time for a segment definition ID qualification. The timestamp must be provided in [ISO 8601](https://datatracker.ietf.org/doc/html/rfc3339#section-5.6) format. |
| `filter.segmentQualificationTime.endTime` | Segment qualification end time for a segment definition ID for a given status. It not provided, there will be no filter on the end time for a segment definition ID qualification. The timestamp must be provided in [ISO 8601](https://datatracker.ietf.org/doc/html/rfc3339#section-5.6) format. |
| `filter.fromIngestTimestamp`| Limits exported profiles to only include those that have been updated after this timestamp. The timestamp must be provided in [ISO 8601](https://datatracker.ietf.org/doc/html/rfc3339#section-5.6) format. <ul><li>`fromIngestTimestamp` for **profiles**, if provided: Includes all the merged profiles where merged updated timestamp is greater than the given timestamp. Supports `greater_than` operand.</li><li>`fromIngestTimestamp` for **events**: All events ingested after this timestamp will be exported corresponding to resultant profile result. This is not the event time itself but the ingestion time for the events.</li> |
...
| `fields` | A list of the exported fields, separated by commas.  |
| `schema.name` | The name of the schema associated with the dataset where data is to be exported. |
| `filter.segments` | The segment definitions that are exported. The following fields are included:<ul><li>`segmentId`: Segment definition ID for profiles to be exported.</li><li>`segmentNs`: Segment definition namespace for the given `segmentID`.</li><li>`status`: An array of strings providing a status filter for the `segmentID`. By default, `status` will have the value `["realized"]` which represents all profiles that fall into the segment definition at the current time. Possible values include: `realized` and `exited`.  A value of `realized` means the profile qualifies for the segment definition. A value of `exiting` means the profile is exiting the segment definition.</li></ul> |
| `mergePolicy` | Merge policy information for the exported data. |
| `metrics.totalTime` | A field indicating the total time that export job took to run. |
| `metrics.profileExportTime` | A field indicating the time it took for the profiles to export. |
