...
| Query that refers to a map | Any segment definition that refers to a map of properties. | People who have added to cart on their external segment. | `chain(xEvent, timestamp, [A: WHAT(eventType = "addToCart") WHERE(externalSegmentMapProperty.values().exists(stringProperty="active"))])` |
