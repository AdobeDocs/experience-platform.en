...
| Single event with a profile attribute | Any segment definition that refers to a single incoming event, with no time restriction, and one or more profile attributes. **Note:** The query will be immediately evaluated when the event comes. However, if a profile event comes in, it will need to wait 24 hours to be incorporated. | ![](../images/ui/streaming-segmentation/profile-hit.png) |
