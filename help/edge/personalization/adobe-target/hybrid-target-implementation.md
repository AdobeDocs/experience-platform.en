...
Hybdrid implementation describes a way of deploying Adobe Target personalization while eliminating page flicker and reducing latency.

This is done by retrieving personalization content server-side, using the [Adobe Experience Platform APIs](https://developer.adobe.com/experience-platform-apis/) and rendering the personalized content on a client-side single-page app (SPA), using the [Web SDK](../../home.md), through the `applyResponse` command.
