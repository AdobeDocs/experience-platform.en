...
| Field | Description |
| --- | --- |
| [!UICONTROL Sandbox] | **(Required)** Select the Platform sandbox that you want to send data to. Sandboxes are virtual partitions in Adobe Experience Platform that allow you to isolate your data and implementations from others in your organization.<br><br>If you create a datastream without selecting a sandbox, you can still select a sandbox later.<br><br>Once a datastream is created and a sandbox was selected, the sandbox cannot be changed. The [!UICONTROL Sandbox] selection field is therefore unavailable when editing an existing datastream with a selected sandbox.<br><br> The [!UICONTROL Sandbox] selection field is therefore unavailable when editing an existing datastream.<br><br>For more details about the role of sandboxes in Experience Platform, see the [sandboxes documentation](../../sandboxes/home.md). |
