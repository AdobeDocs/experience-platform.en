...
This guide requires a working understanding of the various [!DNL Experience Platform] services involved with merge policies. Before beginning this tutorial, or working with Platform APIs, please review the documentation for the following services:
...
Each profile fragment contains information for just one identity out of the total list of identities that could potentially exist for an individual customer. When the identity graph type used results in more than one identity, there is the potential for conflicting profile attributes and priority must be specified. Selecting a merge method allows you to specify which dataset attributes to prioritize if a merge conflict occurs between datasets. 
...
Selecting **[!UICONTROL Dataset precedence]** as the merge method requires you to select Profile datasets and manually order the datasets for precedence. You can select up to 50 datasets from the dataset list. As datasets are selected, they appear on the right-hand side of the screen, allowing you to drag and drop the datasets and order them for precedence. As the datasets are adjusted in the list, the ordinal (1, 2, 3, etc) next to the dataset will update, displaying precedence (1 being given the highest priority, then 2, and onward).
...
Also displayed on this screen is the total number of **[!UICONTROL ExperienceEvent datasets]** created by your organization.

#### Timestamp ordered {#timestamp-ordered-experienceevent}

If you selected **[!UICONTROL Timestamp ordered]** as the merge method for Profile datasets, the attributes from the most recently updated ExperienceEvent datasets will take precedence here as well. Similar to the Profile datasets, timestamp ordering will be applied across all ExperienceEvent datasets.

![](../images/merge-policies/timestamp-experienceevent.png)

#### Dataset precedence {#dataset-precedence-experienceevent}

If you selected **[!UICONTROL Dataset precedence]** as the merge method for Profile datasets, you will need to select ExperienceEvent datasets and manually order the datasets for precedence. You can select up to 50 ExperienceEvent datasets from the dataset list. As datasets are selected, they appear on the right-hand side of the screen, allowing you to drag and drop the datasets and order them for precedence. As the datasets are adjusted in the list, the ordinal (1, 2, 3, etc) next to the dataset will update, displaying precedence (1 being given the highest priority, then 2, and onward).

![](../images/merge-policies/dataset-precedence-experienceevent.png)

### [!UICONTROL Review] {#review}

The final step in the workflow is to review your merge policy. The **[!UICONTROL Review]** screen displays the name of your new merge policy, the [!UICONTROL Schema class] on which it is based, the [!UICONTROL ID stitching] option that you selected, as well as the [!UICONTROL Merge method] and the datasets included in the merge policy. To view all of the Profile or ExperienceEvent datasets included, select the number of datasets to expand the dropdown list.
...
If you selected **[!UICONTROL Dataset precedence]** as the merge method for your merge policy, the lists of Profile and ExperienceEvent datasets include only the Profile and ExperienceEvent datasets that you selected during the creation workflow, respectively. The order of the datasets should match the precedence that you specified during creation. If it does not, use the [!UICONTROL Back] button to return to the previous workflow steps and adjust the precedence.
