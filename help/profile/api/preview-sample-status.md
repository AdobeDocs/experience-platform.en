...
The profile count also includes both profiles with attributes (record data) as well as profiles containing only time series (event) data, such as Adobe Analytics profiles. The sample job is refreshed regularly as [!DNL Profile] data is ingested in order to provide an up-to-date total number of profiles within [!DNL Platform].
...
The following request uses the `date` parameter to return the report for the date specified.
...
You can perform a GET request to the `/previewsamplestatus/report/namespace` endpoint to view the breakdown by identity namespace across all of the merged profiles in your [!DNL Profile] store. Identity namespaces are an important component of Adobe Experience Platform [!DNL Identity Service] that serve as indicators of the context to which customer data relates. To learn more, visit the [identity namespace overview](../../identity-service/namespaces.md).
