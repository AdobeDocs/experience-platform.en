...
| to_map (Syntax 1) | Use this function to create Map data types. |<ul><li>KEY: **Required** Keys must be a string. If any other primitive values such as integers or dates are provided, then they are auto-converted to strings and are treated as strings.</li><li>ANY_TYPE: **Required** Refers to any supported XDM data type except Maps.</li></ul> | to_map(KEY, ANY_TYPE, KEY, ANY_TYPE, ... )| Please see the [appendix](#to_map) for a code sample. | | 
| to_map (Syntax 2) | Use this function to create Map data types.| <ul><li>OBJECT: **Required** You can provide an incoming object or object array and point to an attribute inside the object as key.</li></ul> | to_map(OBJECT) | Please see the [appendix](#to_map) for a code sample. |
| to_map (Syntax 3) | Use this function to create Map data types.| <ul><li>OBJECT: **Required** You can provide an incoming object or object array and point to an attribute inside the object as key.</li></ul> | to_map(OBJECT_ARRAY, ATTRIBUTE_IN_OBJECT_TO_BE_USED_AS_A_KEY) | Please see the [appendix](#to_map) for a code sample. |
...
```json
example = "to_map(\"firstName\", \"John\", \"lastName\", \"Doe\")",
...
```json
example = "to_map(address) where input is " +
...
```json
example = "to_map(addresses) where addresses is\n" +
        "\n" +
        "[\n" +
        "    {\n" +
        "        \"line1\": \"345 park ave\",\n" +
