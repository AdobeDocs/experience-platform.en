...
explode | Splits the string based on a regex and returns an array of parts. Can optionally include regex to split the string. By default, the splitting resolves to ",". The following delimiters **need** to be escaped with `\`: `+, ?, ^, |, ., [, (, {, ), *, $, \` If you include multiple delimiters, the string must contain an **exact match** of the delimiter in order to be split. | <ul><li>STRING: **Required** The string that needs to be split.</li><li>REGEX: *Optional* The regular expression that can be used to split the string.</li></ul> | explode(STRING, REGEX) | explode("Hi, there!", " ") | `["Hi,", "there"]`
