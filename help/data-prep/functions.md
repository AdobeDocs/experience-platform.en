...
| nullify | Sets the value of the attribute to `null`. This should be used when you do not want to copy the field to the target schema. **Important**: Do not use `nullify()` as the return value for a destination field that is marked as required in the target XDM schema. Doing so writes a null value to a required field, causing the row to be rejected. Use a non-null fallback value for required fields instead. | | nullify() | nullify() | `null`  |
