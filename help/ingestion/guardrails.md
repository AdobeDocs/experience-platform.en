...
| --- | --- | --- |
| Data lake ingestion using the batch ingestion API             | <ul><li>Adobe recommends keeping the number of files in a batch below 10,000. Batches containing more than 10,000 files are not guaranteed to be ingested in a timely and efficient manner.</li><li>Adobe recommends keeping the batch size below 100 GB. Batches larger than 100 GB are not guaranteed to be ingested in a timely and efficient manner.</li><li>You can ingest up to 20 GB of data per hour to data lake using the batch ingestion API.</li><li>The maximum number of files per batch is 25,000. Batches containing more than 25,000 files will fail at ingestion time.</li><li>The maximum batch size is 1 TB. Batches larger than 1 TB will fail at ingestion time.</li><li>The maximum CDC batch size is 100 GB.</li><li>The maximum number of properties or fields per row is 10000.</li><li>The maximum number of batches per minute, per user is 2000.</li></ul>                                                                                                                                            | If a batch exceeds an enforced file count or batch size limit, ingestion fails and an error message identifies the exceeded threshold. For example: `Batch ingestion guardrails hit: 25143 files (over the 25000 threshold); 1150000000000 bytes (over the 1099511627776 threshold)` |
| Data lake ingestion using batch sources                       | <ul><li>Adobe recommends keeping the number of files in a batch below 10,000. Batches containing more than 10,000 files are not guaranteed to be ingested in a timely and efficient manner.</li><li>Adobe recommends keeping the batch size below 100 GB. Batches larger than 100 GB are not guaranteed to be ingested in a timely and efficient manner.</li><li>You can ingest up to 200 GB of data per hour to data lake using batch ingestion sources such as [!DNL Azure Blob], [!DNL Amazon S3], and [!DNL SFTP].</li><li>A batch size should be between 256 MB and 1 TB. Batches larger than 1 TB will fail at ingestion time. This applies to both uncompressed and compressed data. When compressed data is uncompressed in the data lake, these limitations will apply.</li><li>The maximum number of files per batch is 25,000. Batches containing more than 25,000 files will fail at ingestion time.</li><li>The minimum size of a file or folder is 1 byte. You cannot ingest 0 byte size files or folders.</li></ul> | Read the [sources overview](../sources/home.md) for a catalog of sources you can use for data ingestion. If a batch exceeds an enforced file count or batch size limit, ingestion fails and an error message identifies the exceeded threshold.                                      |
...
| Upsert batch ingestion   | Ingestion of upsert batches can be up to 10x slower than regular batches, therefore, you should **keep your upsert batches under two million records** in order to ensure an efficient runtime and to avoid blocking other batches from being processed in the sandbox. | While you can ingest batches that exceed two million records, the time of your ingestion will be significantly longer due to the limitations of small sandboxes. |

{style="table-layout:auto"}

>[!NOTE]
>
>The recommended batch size and file count are performance guidelines and are not enforced limits. You can ingest regular batches containing between 10,000 and 25,000 files or larger than 100 GB and up to 1 TB, but these batches are not guaranteed to be ingested in a timely and efficient manner. Batches that exceed an applicable enforced maximum fail at ingestion time.
