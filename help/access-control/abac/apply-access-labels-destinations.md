...
This functionality allows you to apply access labels to destination dataflows. You can ensure that certain users in your organization get access to some destination dataflows, while other users can access other dataflows.

## Prerequisites

access to the permissions UI or talk to an admin
create roles, assign users, etc

## Apply access labels to other Experience Platform resources

While this release enables object-level access to specific destination dataflows, this functionality is already generally available for other resources, such as:
