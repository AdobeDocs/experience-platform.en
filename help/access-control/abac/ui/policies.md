...
To begin configuring a policy, navigate to **[!UICONTROL Permissions]** in [Adobe Experience Cloud](https://experience.adobe.com/){target="_blank"}. Select **[!UICONTROL Policies]** from the left panel. Select the **[!UICONTROL Default-Label-Based-Access-Control-Policy]** from the list.

![The policies workspace showing a list of existing policies.](../../images/ui/policies/policies-home.png){zoomable="yes"}

The policy's details workspace will appear. Select the **[!UICONTROL Sandboxes]**. A list of sandboxes associated with the policy are displayed.
...
>[!IMPORTANT]
>
>By default, the **[!UICONTROL Auto-include]** feature is turned on, which means all current and future sandboxes are automatically added to the policy.

Toggle off the **[!UICONTROL Auto-include]** feature to stop future sandboxes from being automatically added to the policy. Toggling off the feature **will not** remove any sandboxes from the policy.
...
If **[!UICONTROL Auto-include]** is not active in a policy, you can use the toggle to turn it back on. The **[!UICONTROL Enable Auto-include]** dialog appears prompting you to confirm your selection. Select **[!UICONTROL Enable]** to complete the configuration setting. 

>[!NOTE]
>
> Any sandboxes you removed from the policy while **[!UICONTROL Auto-include]** was toggled off will be re-added.
...
The **[!UICONTROL Add Sandboxes]** dialog appears. Select the sandbox(es) you wish to add to the policy and then select **[!UICONTROL Save]**.
