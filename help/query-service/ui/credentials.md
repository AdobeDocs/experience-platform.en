...
>
>- Users must log in with their username and password comprised of `{technicalAccountId}:{credential}`. More information can be found in the [Generate credentials](#generate-credentials) section.
>- By default, non-expiring credentials are granted permissions to execute only `SELECT` queries. If you require the ability to run `CTAS` or `ITAS` queries, you must manually add the "Manage Dataset" and "Manage Schemas" permissions to the role associated with the non-expiring credential.
