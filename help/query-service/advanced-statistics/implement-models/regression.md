...
| `CACHE_NODE_IDS`             | If `false`, the algorithm passes trees to executors to match instances with nodes. If `true`, the algorithm caches node IDs for each instance. Caching can speed up the training of deeper trees. | false         | `true` or `false`                                                                                      |
| `CHECKPOINT_INTERVAL`        | Specifies how often to checkpoint the cached node IDs. For example, `10` means that the cache will be checkpointed every 10 iterations.                                                                  | 10            | (>=1)                                                                                                  |
...
| `MAX_MEMORY_IN_MB`           | The maximum memory, in MB, allocated to histogram aggregation. If the memory is too small, only 1 node is split per iteration, which may cause the aggregates to exceed the size.              | 256           |                                                                                                        |
...
| `MIN_INSTANCES_PER_NODE`      | The minimum number of instances each child must have after a split. If a split results in fewer instances than this value, the split is discarded.                                                   | 1             | (>= 1)                                                                                                |
| `MAX_MEMORY_IN_MB`            | The maximum memory, in MB, allocated to histogram aggregation. If this value is too small, only 1 node is split per iteration, and its aggregates may exceed this size.                        | 256           |                                                                                                      |
