...
| Result Limit | <ul><li>Query Service UI - The number of output rows can be [configured with a UI setting](./ui/user-guide.md#result-count) to between 50-500 rows.</li><li>Third-party clients - 50,000</li><li>[!DNL PostgresSQL] client - 50,000</li></ul> | <ul><li>Query Service UI - The number of output rows can be [configured with a UI setting](./ui/user-guide.md#result-count) to between 50-500 rows.<br>Note: CTAS and ITAS queries only generate success messages as the query output is stored in derived datasets.</li><li>Third-party clients  (no upper limit to rows)</li><li>[!DNL PostgresSQL] client  (no upper limit to rows)</li><li>REST APIs (no upper limit to rows)</li></ul> |
