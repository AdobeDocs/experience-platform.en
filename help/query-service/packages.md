...
- **Compute hours**: Compute hours is the metric used to track the scanning and writing of data with batch queries using the Query Service API. It is calculated in hours per year and is measured across all sandboxes. The amount of Compute hours provided to your organization is defined in the deal scoping process.
...
| Accelerated query and reporting usage | Not available. |  |
| Data lake storage capacity | ? / No | Yes - Your data lake storage capacity is measured in terabytes (TB ) and dependant on the quantity of compute hours that you have bought. |
| Data export allowance | ? / No | Yes - Your annual data export allowance is measured in terabytes (TB ) and dependant on the quantity of compute hours that you have bought. |
...
| Result Limit | <ul><li>Query Service UI - 100 rows</li><li>Third-party client - 50,000</li><li>[!DNL PostgresSQL] client - 50,000</li></ul> | <ul><li>Query Service UI (no upper limit to rows)</li><li>Third-party clients  (no upper limit to rows)</li><li>[!DNL PostgresSQL] client  (no upper limit to rows)</li><li>REST APIs (no upper limit to rows)</li></ul> |
