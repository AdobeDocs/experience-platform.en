...
boolean, date, datetime, text, float, bigint, integer, map, array, struct

```sql
To define the struct type, use ROW (Column_name <data_type> [, column name <data_type> ]*)
for Map -> Map <data_type, data_type>
for Array -> Array <data_type>
```
