...
You can verify that your account details are correct by making a POST request to the `/accounts/verify` endpoint.
...
```shell
curl -X POST https://platform.adobe.io/data/foundation/queryauth/accounts/verify \
 -H 'Authorization: Bearer {ACCESS_TOKEN}' \
 -H 'Content-Type: application/json' \
 -H 'x-gw-ims-org-id: {IMS_ORG}' \
 -H 'x-api-key: {API_KEY}' \
...
A successful response returns HTTP status 200 noting it was successfully verified.
