...
| [!UICONTROL Edge Segmentation] | Enables [edge segmentation](../segmentation/ui/edge-segmentation.md) for this datastream. When the [Web SDK](../web-sdk/home.md) or [Edge Network Server API](../server-api/overview.md) sends data through a datastream with edge segmentation enabled, any updated audience memberships for the profile in question are sent back in the response.<br><br>You can use this option in combination with **[!UICONTROL Personalization Destinations]** for same-page and next-page personalization use cases through [edge destinations](../destinations/ui/activate-edge-personalization-destinations.md) or [!DNL Offer Decisioning].|
...
| [!UICONTROL Target Third Party ID namespace] | The identity namespace for the `mbox3rdPartyId` you want to use for this datastream. If you use [!DNL Customer Attributes] integration with Adobe Target, you must provide a namespace value of your choice. You must use this namespace in the `IdentityMap` section of your XDM schema to send the `customerID` used in your Customer Attributes file uploads. See the guide on [implementing `mbox3rdPartyId` with the Web SDK](../web-sdk/personalization/adobe-target/using-mbox-3rdpartyid.md) for more information. |
