...
If you enable any of the fields above for data collection, make sure that you correctly set the [`context`](/help/edge/commands/configure/context.md) array property when configuring the Web SDK.
...
| [!UICONTROL First Party ID Cookie] | When enabled, this setting tells the Edge Network to refer to a specified cookie when looking up a [first-party device ID](../edge/identity/first-party-device-ids.md), rather than looking up this value in the Identity Map.<br><br>When enabling this setting, you must provide the name of the cookie which should store the ID. |
...
[!UICONTROL Media Analytics] | Enables processing of streaming tracking data for Edge Network integration via Experience Platform SDKs or Media Edge API. Learn about Media Analytics from the [documentation](https://experienceleague.adobe.com/docs/media-analytics/using/media-overview.html).  |
...
This service controls whether and how data is sent to Adobe Analytics. See [Sending data to Adobe Analytics](/help/edge/use-cases/adobe-analytics.md).
