...
All resources within the Schema Library are contained in a specific sandbox within an IMS Organization. In some cases, you may want to share Experience Data Model (XDM) resources between sandboxes and IMS Orgs. The Schema Registry API provides two endpoints that allow you generate an export payload for any schema, mixin, or data type in the Schema Library, and then use that payload to import that resource (and all dependent resources) into a target sandbox and IMS Org.
