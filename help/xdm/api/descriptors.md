...
### Descriptor labelling limitations  {#descriptor-limitations}

You cannot label the tenant namespace object as the system would apply that label to every single custom field across that sandbox. Instead, you must specify the leaf node under that object that you need to label.

Labelling the tenant namespace object results in the fol;lowing error:
