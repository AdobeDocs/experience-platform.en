...
All resources within the [!DNL Schema Library] are contained in a specific sandbox within Adobe Experience Platform. In some cases, you may want to share Experience Data Model (XDM) resources between sandboxes and organizations. The `/rpc/export` endpoint in the [!DNL Schema Registry] API allows you generate an export payload for any schema, schema field group, or data type in the [!DNL Schema Library], and then use that payload to import that resource (and all dependent resources) into a target sandbox and organization through the [`/rpc/import` endpoint](./import.md).
