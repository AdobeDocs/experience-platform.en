...
Finally, confirm that you do not need to rename or reorganize the schema structure. Schema structure changes are limited to additive updates only (see [schema immutability constraints](#why-planning-matters)). Any long-term ambiguity in naming or organization cannot be corrected later, so resolve these issues before enablement.
...
After completing the pre-enablement checklist, follow these steps to enable Profile. As explained in [Understanding the enablement workflow](#why-planning-matters), you must enable the schema before you enable any datasets that use that schema.
...
Repeat this process for each dataset that should contribute to Real-Time Customer Profile. For detailed instructions and API enablement options, refer to the dataset user guide referenced in the Prerequisites section.
