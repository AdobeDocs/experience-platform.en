...
Profile-enabled schemas cannot be reversed. If a schema should no longer be used, rename it to clarify its status and create a replacement schema with the correct structure and identity configuration. This prevents data ingestion into the deprecated schema.
