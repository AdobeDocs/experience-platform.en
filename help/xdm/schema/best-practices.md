...
* **Confirm `_id` isn't used as an identity**: The `_id` field in Experience Event schemas cannot be used as an identity as it is meant for record uniqueness.
...
* **Ensure uniqueness of a selected field**: Your chosen field should be unique compared to the primary identity in the schema. If it is not, do not mark it as an identity. For instance, if multiple customers can provide the same email address, then that namespace is not a suitable identity. This principle also applies to other identity namespaces like phone numbers. Marking a non-unique field as an identity could cause unwanted profile collapse. Using configuration provided by [identity graph linking](../../identity-service/identity-graph-linking-rules/overview.md) rules is reccomended to prevent these issues.
* **Verify minimum string lengths**: All string fields should possess a minimum length of one, as strings should never be empty. Null values for non-required fields, however, are acceptable.
