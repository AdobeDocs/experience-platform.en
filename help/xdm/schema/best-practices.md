...
When ingesting data into the data lake, data validation is only enforced for constrained fields. To validated a particular field during a batch ingestion, you must mark the field as constrained in the XDM schema. To prevent bad data being ingested into Platform, you are recommended to define the criteria for field level validation when creating your schemas.
...
The following are a collection of suggestions for maintaining data integrity when creating a schema:
...
* **Limit eVars in Analytics Schema**: Typically, an Analytics schema should have only one eVar designated as an identity. If you intend to use more than one eVar as an identity, you should double-check whether the data structure can be optimized.
