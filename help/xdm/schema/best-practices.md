...
* **Ensure uniqueness of a selected field**: Your chosen field should be unique compared to the primary identity in the schema. If it is not, do not mark it as an identity. For instance, if multiple customers can provide the same email address, then that namespace is not a suitable identity. This principle also applies to other identity namespaces like phone numbers. Marking a non-unique field as an identity could cause unwanted profile collapse. To prevent data inconsistencies or identity mismatches, use the configuration provided by [identity graph linking](../../identity-service/identity-graph-linking-rules/overview.md) rules.
