...
Use this option to allow the Web SDK to read and write the legacy `mbox` and `mboxEdgeCluster` cookies that are used by the `at.js` 1.x or 2.x libraries. This setting helps keep visitor profiles intact while moving between pages using the Web SDK or `at.js` on the same website. If you do not have `at.js` implemented anywhere on your site, you do not need to enable this checkbox.
...
This drop-down menu is the tag equivalent to [`autoCollectPropositionInteractions.AJO`](/help/collection/js/commands/configure/autocollectpropositioninteractions.md). Its default value is [!UICONTROL Always].
...
This drop-down menu is the tag equivalent to [`autoCollectPropositionInteractions.TGT`](/help/collection/js/commands/configure/autocollectpropositioninteractions.md). Its default value is [!UICONTROL Never].
