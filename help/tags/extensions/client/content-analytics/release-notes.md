...
| Version | Date | Fixes |
|---|---|---|
| 1.0.52 | 27 April 2026 | <ul><li>Added tracking for assets where CSS loads images in the background of an element in the DOM.</li><li>Added a hard-coded `permanentlyBlockedURLs` array containing [maps.googleapis.com](https://maps.googleapis.com) and [mapsresources-pa.googleapis.com](https://mapsresources-pa.googleapis.com). These URLs are always blocked by default in the Content Analytics library.</li><li>Added `idSource` and `channel` fields to XDM data collection requests.</li></ul> |
