...
You can apply a set of labels for an entire dataset by providing them in the payload of a PUT request to the [!DNL Dataset Service] API. You cannot add labels to individual dataset fields.   
...
>[!IMPORTANT]
>
>Use the `optionalLabels` property in a PUT request to remove labels from individual dataset fields. For more information see the section on [removing labels from a dataset](#remove).
