...
This tutorial uses the [!DNL Flow Service] API to explore protocols applications.
...
In order to explore your protocol system using [!DNL Experience Platform] APIs, you must possess a valid base connection ID. If you do not already have a base connection for the protocol system you wish to work with, you can create one through the following tutorial:
...
Using the connection ID for your protocols application, you can explore your data tables by performing GET requests. Use the following call to find the path of the table you wish to inspect or ingest into [!DNL Experience Platform].
...
| --- | --- |
| `{BASE_CONNECTION_ID}` | The ID of a protocol base connection. |
...
A successful response returns an array of tables from your protocols application. Find the table you wish to bring into [!DNL Experience Platform] and take note of its `path` property, as you are required to provide it in the next step to inspect its structure.
