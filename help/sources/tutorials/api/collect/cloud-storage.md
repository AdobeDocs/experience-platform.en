...
<!-- TODO: PLAT-240919 Update CDC Parameter for relational schema Support
Requirements for update:
- Replace existing line 98 parameter description for params.cdcEnabled
- New description: "A boolean value that indicates whether change history capture is enabled or not. When enabled with relational schemas, CDC uses the _change_request_type control column (u=upsert, d=delete) which is read at ingestion time but not stored in the target schema. This property is supported by the following cloud storage sources: <ul><li>[!DNL Azure Blob]</li><li>[!DNL Data Landing Zone]</li><li>[!DNL Google Cloud Storage]</li><li>[!DNL SFTP]</li></ul> For more information, read the guide on using [change data capture in sources](../change-data-capture.md) and [relational schema overview](../../../xdm/schema/relational.md)."
-->
