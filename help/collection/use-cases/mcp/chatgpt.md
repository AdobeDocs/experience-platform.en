...
1. **MCP server (backend)**: Owned by your organization. It implements tools like listing items, fetching details, or submitting requests. In this use case, it sends experience events to the Edge Network and returns business data + Edge response "handles" to the MCP host.
1. **Adobe IMS**: Issues access tokens used by your MCP server to call Adobe data collection APIs.
1. **Adobe Experience Platform Edge Network**: Receives experience events sent by your MCP server. It returns analytics ingestion acknowledgements, state updates (like identity), and personalization decisions.
1. **Embedded web UI (frontend widget rendered by the MCP host)**: Displays structured results and applies Edge personalization received from the MCP server backend.

## Data flow

1. **User** prompts **ChatGPT** using your MCP server.
1. **ChatGPT** interprets the prompt's intent, and calls the appropriate **Backend MCP tool** (server-side).
1. **Backend MCP server** uses the Data collection APIs (`interact` endpoint) to send an experience event to the **Edge Network** (analytics + personalization request).
1. **Edge Network** returns handles (state + decisions) to the **Backend MCP tool**.
1. **Backend MCP tool** returns structured tool output along with `_adobe.handles` to **ChatGPT**.
1. **Frontend widget** renders the result and applies `_adobe.handles` using the Web SDK JavaScript library's `applyResponse` command. This command hydrates client-side state (such as identity and routing cookies) and renders personalization decisions in the UI.
...
## Step 5: MCP server returns structured tool output plus `_adobe.handles` to ChatGPT

Your MCP tool response includes both the structured tool output and a `handles` object, which includes personalization from the Edge Network. This use case places the Edge Network response in `structuredContent._adobe.handles` so that the widget can reliably read from there.
...
```json
{
  ...
  "structuredContent": {
    ...
    "_adobe": {
      "handles": [
        { "type": "state:store", "payload": [{ "key": "kndctr_..._identity", "value": "..." }] },
        { "type": "personalization:decisions", "payload": [{ "id": "..." }] }
      ]
    }
  },
  "_meta": {
    "ui": {
      "resourceUri": "ui://office-details-widget"
    }
  }
}
```

Ensure that your tool output adheres to the following:

* The `structuredContent` object contains content for ChatGPT to include in its response.
* The `_meta` object contains data that you want to hydrate your UI without exposing that data to ChatGPT.
...
The widget renders the structured tool output and uses the Web SDK JavaScript library's [`applyResponse`](../../js/commands/applyresponse.md) command to hydrate the client-side identity and render personalization. Make sure that you call the [`configure`](../../js/commands/configure/overview.md) command at least once on a page before calling `applyResponse`. Since your MCP server performed an `interact` call already, you do not need to call the [`sendEvent`](../../js/commands/sendevent/overview.md) command.

```js
// The Web SDK must be configured on each page load before other commands.
alloy("configure", {
...
* If you use First-Party Device IDs (FPIDs) with the Edge Network, they must comply with UUIDv4 format or they are rejected. See [Using first-party device IDs](../identity/first-party-device-ids.md) for more information.
