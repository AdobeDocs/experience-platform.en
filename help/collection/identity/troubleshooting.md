...
Identity problems often surface as symptoms in downstream reporting — inflated visitor counts, fragmented profiles, or broken personalization — rather than as errors in the implementation itself. This page helps you diagnose and resolve the most common identity issues in Web SDK implementations.
...
### Visitor count inflation {#visitor-inflation}
...
| JavaScript-set FPID cookies | Cookies set via `document.cookie` are subject to browser restrictions (ITP, ETP) that cap their lifetime, sometimes to as little as 24 hours. | Set FPID cookies from the server using a DNS A/AAAA record, not from JavaScript. |

### ECID changes unexpectedly between pages {#ecid-changes}
...
2. Verify that the cookie domain is set broadly enough. A cookie set on `shop.example.com` is not available on `www.example.com`. Ensure first-party collection and cookie-setting infrastructure use the same domain scope.
3. Check for JavaScript that clears cookies on navigation (for example, aggressive cookie-consent scripts or privacy tools).
4. If using an SPA (single-page application), verify that the Web SDK is configured once at app initialization, not re-initialized on every route change. Re-initialization can generate a new ECID.

### FPID is not seeding the ECID {#fpid-not-seeding}
...
1. **Verify the FPID cookie format**: The FPID must be a valid [UUIDv4](https://datatracker.ietf.org/doc/html/rfc4122). Open your browser's developer tools, find the FPID cookie, and confirm the value matches the pattern `xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx`.
2. **Check the cookie name in the datastream**: If you are using the [datastream cookie method](./first-party-device-ids.md#setting-cookie-datastreams), the cookie name configured in the datastream must exactly match the name of the cookie your server sets.
3. **Confirm the cookie is sent on the request**: In the Network tab, inspect the `Cookie` header on the Edge Network request. The FPID cookie must be included.
...
### Cross-domain identity is not working {#cross-domain-not-working}
...
2. **Check the timing**: The `adobe_mc` parameter expires after five minutes. If the destination page takes too long to load (for example, due to redirects or slow network), the parameter may expire before the Web SDK can read it.
...
### Mobile-to-web identity handoff is failing {#mobile-to-web-failing}
...
## Consent and identity {#consent-identity}

Consent choices can directly affect identity behavior. The following situations commonly cause confusion:

| Scenario | Identity behavior |
| --- | --- |
| Consent denied before Web SDK initialization | The Web SDK does not send any requests and does not set identity cookies. No ECID is generated. |
| Consent withdrawn after initial collection | Depending on your [`defaultConsent`](/help/collection/js/commands/configure/defaultconsent.md) and [`setConsent`](/help/collection/js/commands/setconsent.md) configuration, the Web SDK may stop sending events. Existing cookies may remain until they expire, but no new identity requests are made. |
| Consent banner clears all cookies then grants consent | A new ECID is generated because the previous identity cookie was deleted. This creates a new visitor in reporting, even though the person is the same. |

>[!TIP]
>
>If you notice a spike in unique visitor counts after deploying a consent banner, check whether the banner implementation is clearing `kndctr_` cookies before re-granting consent. This is one of the most common causes of post-deployment visitor inflation.

## Validation checklist {#checklist}

Use the following checklist to confirm that identity is working correctly in your implementation:

- [ ] The `kndctr_` identity cookie persists across page loads on the same domain.
- [ ] `getIdentity` returns the same ECID across pages and sessions (when cookies are not cleared).
- [ ] If using FPIDs, the FPID cookie is present on the first Edge Network request and is in UUIDv4 format.
- [ ] Cross-domain links include the `adobe_mc` parameter and the destination resolves identity within five minutes.
- [ ] `identityMap` payloads include exactly one `primary: true` identity.
- [ ] Person-level identifiers use `authenticatedState: "authenticated"` only when the visitor has verified their identity.
- [ ] After logout, the identifier is sent with `authenticatedState: "loggedOut"` rather than being omitted entirely.
- [ ] Consent changes do not inadvertently delete identity cookies.
