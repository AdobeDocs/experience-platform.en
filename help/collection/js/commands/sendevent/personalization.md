...
alloy("sendEvent", {
  personalization: {
    decisionScopes: ["__view__", "hero-banner"],
...
```js
personalization: {
  decisionScopes: ["__view__", "hero-banner", "cart-offer"]
}
```
...
* **`https://ns.adobe.com/personalization/dom-action`**: Offers that are direct DOM actions, typically generated by the Visual Experience Composer in Adobe Target. They typically contain instructions to automatically manipulate elements on the page without additional code. It is the standard for auto-rendered web personalization.
* **`https://ns.adobe.com/personalization/html-content-item`**: Offers that contain raw HTML delivered as a string. Your implementation typically inserts this content within the right location on the page via code. It is common for banner, snippet, or modal content when you want more control than with DOM actions.
* **`https://ns.adobe.com/personalization/json-content-item`**: Offers that are formatted as a JSON object. They are most commonly used in API implementations, or front-ends that expect structured data instead of HTML or DOM changes.
* **`https://ns.adobe.com/personalization/redirect-item`**: Offers that redirect to a different URL. It is used when you want the experience to take the user to a new page based on targeting or decisioning, such as promotional landing pages or personalized onboarding flows.
* **`https://ns.adobe.com/personalization/default-content-item`**: Fallback or default content, typically when no personalized offer is eligible. It ensures that designated content appears for users who don't qualify for any specific activity, keeping the experience consistent for all users (regardless of personalization eligibility).
