...
To verify if data has been exported successfully to the [!DNL Moengage] destination, go to the user profile on your [!DNL Moengage] account. Here, you should find a user attribute named AEPSegments and the other custom attributes that have been mapped in the earlier steps in Adobe Experience Platform.

AEPSegments is an array type attribute that is automatically created. It lists all of the audience names the user is currently associated with in your Adobe Experience Platform workspace."
