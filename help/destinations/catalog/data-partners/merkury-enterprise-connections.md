...
This tutorial provides steps for creating a *Merkury Enterprise Identity Resolution* destination connector using the Platform user interface.
...
In the Platform UI, select **Destinations** from the left navigation bar to
access the Destinations workspace. The Catalog screen displays a variety of
destinations for which you can create an account with.

You can select the Data Partners category from the catalog on the left-hand
