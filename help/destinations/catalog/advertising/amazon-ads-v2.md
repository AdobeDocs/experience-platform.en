...
>
>After the end of April 2026, the [!DNL Amazon Ads] destination card will replace the legacy card, leaving a single destination card in the catalog. Existing legacy dataflows will continue to work and you can manage them in the **[!UICONTROL Browse]** tab beyond that date.
...
An advertiser wants to understand the aggregate impact of their advertising campaigns across Amazon inventory. Using the [!DNL Amazon Ads] destination, the advertiser can pass audiences from [!DNL Real-Time CDP] to [!DNL Amazon Marketing Cloud] ([!DNL AMC]), join the audience data with [!DNL Amazon Ads] signals, and conduct custom analytics on topics such as media impact, audience overlap, and customer journeys in a privacy-compliant format. For example, the advertiser can upload a list of existing customers to understand aggregate advertising campaign performance, or aggregated statistics of on-Amazon conversion events, such as viewing a product detail page, adding a product to a shopping cart, or purchasing a product.

Advertisers can also use [!DNL Amazon Marketing Cloud] SQL to perform overlap analysis, suppressions, additions, or optimizations to audiences on a recurring basis before creating an activation-ready audience in [!DNL Amazon DSP] for targeting.
