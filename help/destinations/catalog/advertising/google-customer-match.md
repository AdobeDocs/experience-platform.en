...
The video below also demonstrates the steps to configure a social destination and activate segments. The video uses LinkedIn as an example, but the steps are similar across social destinations, including [!DNL Google Customer Match]. The steps in the video are also laid out sequentially in the next sections.
