...
|`segmentNamespaceDenyList`| Array | Optional | Restricts users from mapping audiences from the audience namespaces defined in the array to the destination. <br><br>Adobe recommends to allow the export of all audiences, regardless of the origin, by setting `"segmentNamespaceDenyList":[]`. <br><br>**Important:** When creating a destination, if you do not provide `segmentNamespaceDenyList` in the `schemaConfig`, the system will automatically set it to `[]` to prevent lost audiences in the future. For safety, Adobe recommends manually setting `"segmentNamespaceDenyList":[]` in your configuration. <br><br>`segmentNamespaceAllowList` and `segmentNamespaceDenyList` are mutually exclusive. |
