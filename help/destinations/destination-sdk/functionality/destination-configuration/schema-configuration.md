...
|`segmentNamespaceDenyList`| Array | Optional | Restricts users from mapping audiences from the audience namespaces defined in the array to the destination. <br><br>To allow the export of all audiences, regardless of the origin, set `"segmentNamespaceDenyList":[]`. <br><br>If both `segmentNamespaceAllowed` and `segmentNamespaceDenyList` are missing from your configuration, users will only be able to export audiences originating from the [Segmentation Service](../../../../segmentation/home.md). <br><br>`segmentNamespaceAllowList` and `segmentNamespaceDenyList` are mutually exclusive. |
