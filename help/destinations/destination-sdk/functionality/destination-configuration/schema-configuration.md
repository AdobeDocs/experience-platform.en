...
|`segmentNamespaceAllowed`| Array | Optional | Defines specific audience namespaces from which users can map audiences to the destination. Users can only map audiences from the namespaces defined in the array. Cannot be used together with `segmentNamespaceDenied`.<br> <br> Example: `"segmentNamespaceAllowed": ["AudienceManager"]` will allow users to map only audiences from the `AudienceManager` namespace to this destination. |
|`segmentNamespaceDenied`| Array | Optional | Restricts users from mapping audiences to the destination, from the audience namespaces defined in the array. Cannot be used together with `segmentNamespaceAllowed`. <br> <br> Example: `"segmentNamespaceDenied": ["AudienceManager"]` will block users from mapping audiences from the `AudienceManager` namespace to this destination. |
